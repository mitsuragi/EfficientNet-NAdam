...
| ----------- | ------------ | ----------- | ------------- | ----- | ---------- |
| 1           | Да           | Adam        | 1e-4          | 30    | 64         |
| 2           | Нет          | Adam        | 1e-4          | 30    | 64         |
| 3           | Да           | NAdam       | 1e-4          | 30    | 64         |
...
| -------------------------------------- | ---------- | ---------- | ---------- | ---------- | ------------ |
| EfficientNet + Adam, **предобученная** | **0.6878** | **0.6956** | **0.6840** | **0.6831** | **0.7183**   |
| EfficientNet + Adam, обучение с нуля   | 0.4279     | 0.4213     | 0.4243     | 0.4170     | 0.4469       |
| EfficientNet + NAdam, предобученная    | 0.6850     | 0.6951     | 0.6814     | 0.6806     | 0.7131       |

### 3.3. Анализ результатов обучения

#### Эксперимент 1: EfficientNet + Adam, предобученная модель

- **Начальная точность** (эпоха 1): Train Acc = 0.0486, Val Acc = 0.1702
- **Финальная точность** (эпоха 30): Train Acc = 0.8564, Val Acc = 0.7124
- **Тестовая точность**: 0.6878
- **Наблюдения**:
...
- **Начальная точность** (эпоха 1): Train Acc = 0.0123, Val Acc = 0.0126
- **Финальная точность** (эпоха 30): Train Acc = 0.7113, Val Acc = 0.4469
- **Тестовая точность**: 0.4272
- **Наблюдения**:
...
- **Начальная точность** (эпоха 1): Train Acc = 0.0478, Val Acc = 0.1623
- **Финальная точность** (эпоха 30): Train Acc = 0.8480, Val Acc = 0.7015
...
| --------- | ----------------------- | ------ | ------- |
| Accuracy  | 0.6671                  | 0.2573 | +0.4098 |
| Precision | 0.6827                  | 0.2551 | +0.4276 |
| Recall    | 0.6626                  | 0.2534 | +0.4092 |
| F1-score  | 0.6616                  | 0.2382 | +0.4234 |

**Вывод**: Предобучение дает значительное преимущество (+41% по Accuracy), что подтверждает важность transfer learning для задач компьютерного зрения.
...
1. **Лучшая модель**: EfficientNet с предобученными весами и оптимизатором Adam показала наилучшие результаты:
   - Accuracy: **0.6878** (68.78%)
   - F1-score: **0.6956** (69.56%)
   - Precision: **0.684** (68.4%)
   - Recall: **0.6831** (68.31%)

2. **Критическая важность transfer learning**: 
- Использование предобученных весов даёт прирост порядка **+41% по Accuracy** по сравнению с обучением с нуля
- Предобученная модель демонстрирует резкий рост точности уже в первые эпохи (≈46% на train к 3-й эпохе)
- Обучение с нуля даже после 30 эпох достигает лишь **~45% Accuracy** на тестовой выборке

3. **Влияние оптимизатора**:
- Adam и Nadam показывают схожую динамику обучения при fine-tuning предобученной модели
- Adam обеспечивает **незначительно более высокие значения валидационной и тестовой точности**
